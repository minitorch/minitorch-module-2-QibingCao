...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ctx.save_for_backward(a.shape)  # save old shape information
        new_tensor_data = a._tensor.permute(list(order._tensor._storage))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        return (
            minitorch.Tensor.make(
                grad_output._tensor._storage, old_shape, backend=grad_output.backend
            ),
            0.0,
        )
